    ...
    <div class="i-head">
      <div class="shadowMask" v-bind:style="{ opacity:game.key1>0.5?0.5:game.key1 }"></div>
        <div class="head_main">
          ...
          <!-- 搜索 -->
            <div class="i_searchs" v-bind:style="{ opacity:game.key1<0.7?1:game.key1<0.9?1-(game.key1-0.7)*5:0,transform:'scale('+(game.key1<0.7?1:game.key1<0.9?1-(game.key1-0.7)/5:0),width:fontSize * 5.975+'px'}">
                <div class="search">
        ...
        <swipe class="my-swipe1" ref="mySwipe1" @change="changeSwipe" :auto="3000" :continuous="true" :show-indicators="true" v-bind:style="{ opacity:game.key1<0.45?1:game.key1<0.7?1-(game.key1-0.45)*4:0,transform:'scale('+(game.key1<0.45?1:game.key1<0.7?1-(game.key1-0.45)/5:0)+')' }">
...
<style>
    ...
    .my-swipe1 li span{text-align:center;color: #1A1A1A;margin-top:.1rem;display:block;font-size: .22rem;width: .88rem;line-height:.23rem;height:.24rem;overflow: hidden;white-space: nowrap;}
